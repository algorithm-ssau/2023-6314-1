...
docker rmi $(docker images | grep -v "postgres" | grep -v "confluentinc/cp-kafka" | grep -v "confluentinc/cp-zookeeper" | grep -v "obsidiandynamics/kafdrop" | grep -v "config-service"  | grep -v "discovery-service" | awk 'NR>1 {print $1}')
